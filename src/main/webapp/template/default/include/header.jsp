...
<%  UserLoginVo user=(UserLoginVo) request.getSession().getAttribute("user_session_key"); %>
...
<div class="herder_top_big">
   ...
   <ul>
     <li class="code_l"><i class="icon-user"></i><a href="#">${sessionScope.user_session_key.staffName}</a> ｜
     <p style="display:none">
     <span class="code"><img src="${_template_path}/images/code_jiao.png"></span>
     <span><a href="${_template_path}/index/index">个人中心</a></span>
     <span><a href="javascript:void(0);" onclick="showpwd()">修改密码</a></span>
    </p>
   </li>
   <li><p><a href="${_base}/logout">退出</a></p></li>
...
<div class="header1" style=" position:relative;">
 <div class="head1">
  <div class="logo1"><a href="#"><img src="${_template_path}/images/logo.png"  /></a></div>
  <div class="nav1">
   <div class="nava1">
   <div class="yax1" style="width:140px;">
    <p>
     <img src="${_template_path}/images/i_city.png"><span><%=user.getTenantName() %></span>
...
function modifypwd(){
	if(checkPwd()){
		if(verifypwd()){
			var url = "${_template_path}/index/modifyPwd";
			ajaxController.ajax({
		        method: "POST",
		        url: url,
		        dataType: "json",
...
function verifypwd(){
	...
	if($.trim($("#oldPwd").val())!=""){
		$('#oldPwd_error').html("");
		var url = "${_template_path}/index/verifyPwd";
		ajaxController.ajax({
	        method: "POST",
	        url: url,
	        dataType: "json",
		...
		success : function(data) {
			...
			$.each(json, function(index, item) {
				...
				if(hasthreed)
					{
					    ...
					    str+= "<div class='ther1' style='display:none;'><img src='${_template_path}/images/top_1.png'></div>";
				...
				$.each(obj,function(index,item){
					...
					if(thred=='')
						{
					str+="<li><a onmouseover=show('a') href='${_base}/"+obj[index].menuUrl+"'>"+obj[index].menuName+"</a></li>";
						}
					else
						{
					...
					$.each(thred,function(index,item){
						ol+="<li><a href='${_base}/"+thred[index].menuUrl+"'>"+thred[index].menuName+"</a></li>";
					});
					ol+="</ol>"
				});
