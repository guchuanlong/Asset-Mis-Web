 $(document).ready(function () {
        ...
        $("#provinceSelect").val(gn_depart.selectedProvinceCode);
       /* var parentDepartId = $("#parentDepartId").val();
        if(parentDepartId !="0"){
        	$('#provinceSelect').prop('disabled', true);
        }*/
        updateCity();
        $("#cityCode").val(gn_depart.selectedCityCode);
        if(gn_depart.selectedAreaCode!="00"){
        	updateDomainCode();
        	$("#areaCode").val(gn_depart.selectedAreaCode);
        }
            ...
            $.get(_base+'/maintain/depart/tree', function (data) {
                ...
                var options={
        				...
        				callback:function () {
        					...
        					var ydepartId = $("#departId").val();
                            if(departId==ydepartId){
                            	messageController.alert("上级部门不能是本部门，请重新选择！");
                         	   return false;
                            }
        ...
        function updateDomainCode() {
            ...
            ajaxController.ajax({
                ...
                success: function (data) {
                    ...
                    var htmlOutput = template.render(data.data);
                   // if(htmlOutput==""){
                    	// $("#areaCode option:first").before("<option value='0000'>请选择</option>");
                    	// $("#areaCode").val("0000");
                    //}else{
                    	$("#areaCode").empty();
...
function updateDepart(){
    		...
    		var url = _base+"/maintain/depart/editDepart";
    		var param;
    		param={
